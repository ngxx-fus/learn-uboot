 ...
 *
 * Driver for Broadcom GENETv5 Ethernet controller (as found on the Nfus4)
 * This driver is based on the Linux driver:
 *      drivers/net/ethernet/broadcom/genet/nfgenet.c
...
/* Body(1500) + EH_SIZE(14) + VLANTAG(4) + NFSTAG(6) + FCS(4) = 1528.
 * 1536 is multiple of 256 bytes
 */
#define ENET_NFS_TAG_LEN		6
#define ENET_PAD			8
#define ENET_MAX_MTU_SIZE		(ETH_DATA_LEN + ETH_HLEN +	 \
					 VLAN_HLEN + ENET_NFS_TAG_LEN + \
...
static void nfgenet_umac_reset(struct nfgenet_eth_priv *priv)
{
	u32 reg;
...
static int nfgenet_gmac_write_hwaddr(struct udevice *dev)
{
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
...
static void nfgenet_disable_dma(struct nfgenet_eth_priv *priv)
{
...
static void nfgenet_enable_dma(struct nfgenet_eth_priv *priv)
{
...
static int nfgenet_gmac_eth_send(struct udevice *dev, void *packet, int length)
{
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
...
static int nfgenet_gmac_eth_recv(struct udevice *dev,
...
static int nfgenet_gmac_free_pkt(struct udevice *dev, uchar *packet,
				  int length)
{
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
...
static void rx_descs_init(struct nfgenet_eth_priv *priv)
{
...
static void rx_ring_init(struct nfgenet_eth_priv *priv)
{
...
static void tx_ring_init(struct nfgenet_eth_priv *priv)
{
	writel(DMA_MAX_BURST_LENGTH,
...
static int nfgenet_adjust_link(struct nfgenet_eth_priv *priv)
{
	...
	case SPEED_10:
		...
		break;
	default:
		printf("nfgenet: Unsupported PHY speed: %d\n", phy_dev->speed);
		return -EINVAL;
	}
...
static int nfgenet_gmac_eth_start(struct udevice *dev)
{
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
	int ret;
	...
	nfgenet_umac_reset(priv);

	nfgenet_gmac_write_hwaddr(dev);

	/* Disable RX/TX DMA and flush TX queues */
	nfgenet_disable_dma(priv);
	...
	/* Enable RX/TX DMA */
	nfgenet_enable_dma(priv);

	/* read PHY properties over the wire from generic PHY set-up */
	ret = phy_startup(priv->phydev);
	if (ret) {
		printf("nfgenet: PHY startup failed: %d\n", ret);
		return ret;
	}

	/* Update MAC registers based on PHY property */
	ret = nfgenet_adjust_link(priv);
	if (ret) {
		printf("nfgenet: adjust PHY link failed: %d\n", ret);
		return ret;
	}
...
static int nfgenet_phy_init(struct nfgenet_eth_priv *priv, void *dev)
{
	struct phy_device *phydev;
	int ret;
...
static void nfgenet_mdio_start(struct nfgenet_eth_priv *priv)
{
	setbits_32(priv->mac_reg + MDIO_CMD, MDIO_START_BUSY);
}

static int nfgenet_mdio_write(struct mii_dev *bus, int addr, int devad,
			       int reg, u16 value)
{
	...
	nfgenet_mdio_start(priv);

	return wait_for_bit_32(priv->mac_reg + MDIO_CMD,
			       MDIO_START_BUSY, false, 20, true);
}

static int nfgenet_mdio_read(struct mii_dev *bus, int addr, int devad, int reg)
{
	struct udevice *dev = bus->priv;
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
	u32 val;
...
static int nfgenet_mdio_init(const char *name, struct udevice *priv)
{
	...
	bus->read = nfgenet_mdio_read;
	bus->write = nfgenet_mdio_write;
...
/* We only support RGMII (as used on the Nfus4). */
static int nfgenet_interface_set(struct nfgenet_eth_priv *priv)
{
...
static int nfgenet_eth_probe(struct udevice *dev)
{
	struct eth_pdata *pdata = dev_get_plat(dev);
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
	ofnode mdio_node;
	...
	ret = nfgenet_interface_set(priv);
	if (ret)
		return ret;
	...
	ret = nfgenet_mdio_init(name, dev);
	if (ret)
		return ret;

	priv->bus = miiphy_get_dev_by_name(name);

	return nfgenet_phy_init(priv, dev);
}

static void nfgenet_gmac_eth_stop(struct udevice *dev)
{
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
...
static const struct eth_ops nfgenet_gmac_eth_ops = {
	.start                  = nfgenet_gmac_eth_start,
	.write_hwaddr           = nfgenet_gmac_write_hwaddr,
	.send                   = nfgenet_gmac_eth_send,
	.recv                   = nfgenet_gmac_eth_recv,
	.free_pkt               = nfgenet_gmac_free_pkt,
	.stop                   = nfgenet_gmac_eth_stop,
};

static int nfgenet_eth_of_to_plat(struct udevice *dev)
{
	struct eth_pdata *pdata = dev_get_plat(dev);
	struct nfgenet_eth_priv *priv = dev_get_priv(dev);
...
/* The NF2711 implementation has a limited burst length compared to a generic
 * GENETv5 version, but we go with that shorter value (8) in both cases, for
 * the sake of simplicity.
 */
static const struct udevice_id nfgenet_eth_ids[] = {
	{.compatible = "nfs,genet-v5"},
	{.compatible = "nfs,nf2711-genet-v5"},
	{}
};

U_BOOT_DRIVER(eth_nfgenet) = {
	.name   = "eth_nfgenet",
	.id     = UCLASS_ETH,
	.of_match = nfgenet_eth_ids,
	.of_to_plat = nfgenet_eth_of_to_plat,
	.probe  = nfgenet_eth_probe,
	.ops    = &nfgenet_gmac_eth_ops,
	.priv_auto	= sizeof(struct nfgenet_eth_priv),
