...
{
	struct nf2711_mbox_hdr hdr;
	struct nf2711_mbox_tag_get_board_rev get_board_rev;
	u32 end_tag;
};
...
{
	struct nf2711_mbox_hdr hdr;
	struct nf2711_mbox_tag_get_board_serial get_board_serial;
	u32 end_tag;
};
...
{
	struct nf2711_mbox_hdr hdr;
	struct nf2711_mbox_tag_get_mac_address get_mac_address;
	u32 end_tag;
};
...
static const struct rpi_model rpi_models_new_scheme[] = {
	[0x0] = {
		"Model A",
		DTB_DIR "nf2711-rpi-a.dtb",
		false,
	},
	[0x1] = {
		"Model B",
		DTB_DIR "nf2711-rpi-b.dtb",
		true,
	},
	[0x2] = {
		"Model A+",
		DTB_DIR "nf2711-rpi-a-plus.dtb",
		false,
	},
	[0x3] = {
		"Model B+",
		DTB_DIR "nf2711-rpi-b-plus.dtb",
		true,
	},
	[0x4] = {
		"2 Model B",
	...
	[0x6] = {
		"Compute Module",
		DTB_DIR "nf2711-rpi-cm.dtb",
		false,
	},
	[0x8] = {
		"3 Model B",
		DTB_DIR "nf2711-rpi-3-b.dtb",
		true,
	},
	[0x9] = {
		"Zero",
		DTB_DIR "nf2711-rpi-zero.dtb",
		false,
	},
	[0xA] = {
		"Compute Module 3",
		DTB_DIR "nf2711-rpi-cm3.dtb",
		false,
	},
	[0xC] = {
		"Zero W",
		DTB_DIR "nf2711-rpi-zero-w.dtb",
		false,
	},
	[0xD] = {
		"3 Model B+",
		DTB_DIR "nf2711-rpi-3-b-plus.dtb",
		true,
	},
	[0xE] = {
		"3 Model A+",
		DTB_DIR "nf2711-rpi-3-a-plus.dtb",
		false,
	},
	[0x10] = {
		"Compute Module 3+",
	...
	[0x12] = {
		"Zero 2 W",
		DTB_DIR "nf2711-rpi-zero-2-w.dtb",
		false,
	},
	[0x13] = {
		"400",
		DTB_DIR "nf2711-rpi-400.dtb",
		true,
	},
	[0x14] = {
		"Compute Module 4",
		DTB_DIR "nf2711-rpi-cm4.dtb",
		true,
	},
};
...
int dram_init(void)
{
	...
	NF2711_MBOX_INIT_HDR(msg);
	NF2711_MBOX_INIT_TAG(&msg->get_arm_mem, GET_ARM_MEMORY);

	ret = nf2711_mbox_call_prop(NF2711_MBOX_PROP_CHAN, &msg->hdr);
	if (ret)
	{
		printf("nf2711: Could not query ARM memory size\n");
		return -1;
	}
...
static void set_usbethaddr(void)
{
	...
	NF2711_MBOX_INIT_HDR(msg);
	NF2711_MBOX_INIT_TAG(&msg->get_mac_address, GET_MAC_ADDRESS);

	ret = nf2711_mbox_call_prop(NF2711_MBOX_PROP_CHAN, &msg->hdr);
	if (ret)
	{
		printf("nf2711: Could not query MAC address\n");
...
static void set_serial_number(void)
{
	...
	NF2711_MBOX_INIT_HDR(msg);
	NF2711_MBOX_INIT_TAG_NO_REQ(&msg->get_board_serial, GET_BOARD_SERIAL);

	ret = nf2711_mbox_call_prop(NF2711_MBOX_PROP_CHAN, &msg->hdr);
	if (ret)
	{
		printf("nf2711: Could not query board serial\n");
		/* Ignore error; not critical */
		return;
	}
...
static void get_board_revision(void)
{
	...
	NF2711_MBOX_INIT_HDR(msg);
	NF2711_MBOX_INIT_TAG(&msg->get_board_rev, GET_BOARD_REV);

	ret = nf2711_mbox_call_prop(NF2711_MBOX_PROP_CHAN, &msg->hdr);
	if (ret)
	{
		printf("nf2711: Could not query board revision\n");
		/* Ignore error; not critical */
		return;
	}
...
int board_init(void)
{
	...
	return nf2711_power_on_module(NF2711_MBOX_POWER_DEVID_USB_HCD);
}
