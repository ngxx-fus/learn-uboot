 ...
 * and is based on drivers/mmc/host/nf2711.c in Linux which is written by
 *  Phil Elwell <phil@raspberrypi.org>
 *  Copyright (C) 2015-2016 Raspberry Pi (Trading) Ltd.
 * which is based on
 *  mmc-nf2711.c by Gellert Weisz
 * which is, in turn, based on
 *  sdhci-bcm2708.c by Broadcom
 *  sdhci-nf2711.c by Stephen Warren and Oleksandr Tymoshenko
...
struct nf2711_host {
	...
	struct udevice		*dev;
	struct mmc		*mmc;
	struct nf2711_plat	*plat;
	unsigned int		firmware_sets_cdiv:1;
};

static void nf2711_dumpregs(struct nf2711_host *host)
{
...
static void nf2711_reset_internal(struct nf2711_host *host)
{
	u32 temp;
...
static int nf2711_wait_transfer_complete(struct nf2711_host *host)
{
...
static int nf2711_transfer_block_pio(struct nf2711_host *host, bool is_read)
{
...
static int nf2711_transfer_pio(struct nf2711_host *host)
{
	u32 sdhsts;
	bool is_read;
	int ret = 0;
	...
	ret = nf2711_transfer_block_pio(host, is_read);
	if (ret)
		return ret;
...
static void nf2711_prepare_data(struct nf2711_host *host, struct mmc_cmd *cmd,
...
static u32 nf2711_read_wait_sdcmd(struct nf2711_host *host)
{
	u32 value;
	int ret;
...
static int nf2711_send_command(struct nf2711_host *host, struct mmc_cmd *cmd,
				struct mmc_data *data)
{
	u32 sdcmd, sdhsts;
	...
	sdcmd = nf2711_read_wait_sdcmd(host);
	...
	nf2711_prepare_data(host, cmd, data);
...
static int nf2711_finish_command(struct nf2711_host *host)
{
...
static int nf2711_check_cmd_error(struct nf2711_host *host, u32 intmask)
{
	int ret = -EINVAL;
...
static int nf2711_check_data_error(struct nf2711_host *host, u32 intmask)
{
	int ret = 0;
...
static int nf2711_transmit(struct nf2711_host *host)
{
	u32 intmask = readl(host->ioaddr + SDHSTS);
	int ret;

	/* Check for errors */
	ret = nf2711_check_data_error(host, intmask);
	if (ret)
		return ret;

	ret = nf2711_check_cmd_error(host, intmask);
	if (ret)
		return ret;
	...
	if (host->use_busy && (intmask & SDHSTS_BUSY_IRPT)) {
		...
		host->use_busy = false;
		nf2711_finish_command(host);
	}

	/* Handle PIO data transfer */
	if (host->data) {
		ret = nf2711_transfer_pio(host);
		if (ret)
			return ret;
		host->blocks--;
		if (host->blocks == 0) {
			/* Wait for command to complete for real */
			ret = nf2711_wait_transfer_complete(host);
			if (ret)
				return ret;
			/* Transfer complete */
			host->data = NULL;
...
static void nf2711_set_clock(struct nf2711_host *host, unsigned int clock)
{
	int div;
	u32 clock_rate[2] = { 0 };
	...
	if (host->firmware_sets_cdiv) {
		nf2711_set_sdhost_clock(clock, &clock_rate[0], &clock_rate[1]);
...
static int nf2711_send_cmd(struct udevice *dev, struct mmc_cmd *cmd,
			    struct mmc_data *data)
{
	struct nf2711_host *host = dev_get_priv(dev);
	u32 edm, fsm;
	int ret = 0;
...
static int nf2711_set_ios(struct udevice *dev)
{
	struct nf2711_host *host = dev_get_priv(dev);
	struct mmc *mmc = mmc_get_mmc_dev(dev);

	if (!mmc->clock || mmc->clock != host->clock) {
		nf2711_set_clock(host, mmc->clock);
...
static void nf2711_add_host(struct nf2711_host *host)
{
	...
	nf2711_reset_internal(host);
}

static int nf2711_probe(struct udevice *dev)
{
	struct nf2711_plat *plat = dev_get_plat(dev);
	struct nf2711_host *host = dev_get_priv(dev);
	struct mmc *mmc = mmc_get_mmc_dev(dev);
	...
	host->max_clk = nf2711_get_mmc_clock(BCM2835_MBOX_CLOCK_ID_CORE);

	nf2711_set_sdhost_clock(0, &clock_rate[0], &clock_rate[1]);
	host->firmware_sets_cdiv = (clock_rate[0] != ~0);

	nf2711_add_host(host);
...
static const struct udevice_id nf2711_match[] = {
	{ .compatible = "ngxxfus,nf2711-sdhost" },
	{ }
};

static const struct dm_mmc_ops nf2711_ops = {
	.send_cmd = nf2711_send_cmd,
	.set_ios = nf2711_set_ios,
};

static int nf2711_bind(struct udevice *dev)
{
	struct nf2711_plat *plat = dev_get_plat(dev);
...
U_BOOT_DRIVER(nf2711_sdhost) = {
	.name = "nf2711-sdhost",
	.id = UCLASS_MMC,
	.of_match = nf2711_match,
	.bind = nf2711_bind,
	.probe = nf2711_probe,
	.priv_auto	= sizeof(struct nf2711_host),
	.plat_auto	= sizeof(struct nf2711_plat),
	.ops = &nf2711_ops,
};
