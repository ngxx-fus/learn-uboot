...
{
	struct nf2711_mbox_regs *regs =
		(struct nf2711_mbox_regs *)NF2711_MBOX_PHYSADDR;
	ulong endtime = get_timer(0) + TIMEOUT;
	u32 val;

	debug("time: %lu timeout: %lu\n", get_timer(0), endtime);

	if (send & NF2711_CHAN_MASK) {
	...
	for (;;) {
		...
		if (val & NF2711_MBOX_STATUS_RD_EMPTY)
			break;
	...
	for (;;) {
		...
		if (!(val & NF2711_MBOX_STATUS_WR_FULL))
			break;
		if (get_timer(0) >= endtime) {
	...
	val = NF2711_MBOX_PACK(chan, send);
	...
	if (NF2711_MBOX_UNPACK_CHAN(val) != chan) {
		printf("mbox: Response channel mismatch\n");
		return -1;
	}

	*recv = NF2711_MBOX_UNPACK_DATA(val);

	return 0;
}

#ifdef DEBUG
void dump_buf(struct nf2711_mbox_hdr *buffer)
{
	u32 *p;
	u32 words;
	int i;
...
int nf2711_mbox_call_prop(u32 chan, struct nf2711_mbox_hdr *buffer)
{
	int ret;
	u32 rbuffer;
	struct nf2711_mbox_tag_hdr *tag;
	int tag_index;
	...
	if (buffer->code != NF2711_MBOX_RESP_CODE_SUCCESS) {
	...
	tag_index = 0;
	while (tag->tag) {
		if (!(tag->val_len & NF2711_MBOX_TAG_VAL_LEN_RESPONSE)) {
		 ...
		 */
		tag->val_len &= ~NF2711_MBOX_TAG_VAL_LEN_RESPONSE;
