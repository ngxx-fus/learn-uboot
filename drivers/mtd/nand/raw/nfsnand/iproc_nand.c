...
static bool iproc_nand_intc_ack(struct nfsnand_soc *soc)
{
	struct iproc_nand_soc *priv =
			container_of(soc, struct iproc_nand_soc, soc);
	void __iomem *mmio = priv->ext_base + IPROC_NAND_CTLR_READY_OFFSET;
	u32 val = nfsnand_readl(mmio);

	if (val & IPROC_NAND_CTLR_READY) {
		nfsnand_writel(IPROC_NAND_CTLR_READY, mmio);
		return true;
	}
...
static void iproc_nand_intc_set(struct nfsnand_soc *soc, bool en)
{
	struct iproc_nand_soc *priv =
			container_of(soc, struct iproc_nand_soc, soc);
	...
	nfsnand_writel(val, mmio);
}

static void iproc_nand_apb_access(struct nfsnand_soc *soc, bool prepare,
				  bool is_param)
{
	struct iproc_nand_soc *priv =
		container_of(soc, struct iproc_nand_soc, soc);
...
static int iproc_nand_probe(struct udevice *dev)
{
	...
	return nfsnand_probe(pdev, soc);
}

static const struct udevice_id iproc_nand_dt_ids[] = {
	{
		.compatible = "nfs,nand-iproc",
	},
	{ /* sentinel */ }
};
