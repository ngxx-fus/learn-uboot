...
static bool bcm6838_nand_intc_ack(struct nfsnand_soc *soc)
{
...
static void bcm6838_nand_intc_set(struct nfsnand_soc *soc, bool en)
{
	struct bcm6838_nand_soc *priv =
			container_of(soc, struct bcm6838_nand_soc, soc);
	void __iomem *mmio = priv->base + BCM6838_NAND_INT;
...
static int bcm6838_nand_probe(struct udevice *dev)
{
	...
	nfsnand_writel(0, priv->base + BCM6838_NAND_INT);
	nfsnand_writel(BCM6838_NAND_STATUS_MASK,
