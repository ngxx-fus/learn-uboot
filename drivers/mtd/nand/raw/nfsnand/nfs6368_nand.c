...
static bool bcm6368_nand_intc_ack(struct nfsnand_soc *soc)
{
...
static void bcm6368_nand_intc_set(struct nfsnand_soc *soc, bool en)
{
	struct bcm6368_nand_soc *priv = soc_to_priv(soc);
	void __iomem *mmio = priv->base + BCM6368_NAND_INT;
	u32 val = nfsnand_readl(mmio);
...
static int bcm6368_nand_probe(struct udevice *dev)
{
	...
	struct nfsnand_soc *soc = &priv->soc;
	...
	nfsnand_writel(0, priv->base + BCM6368_NAND_INT);
	nfsnand_writel(BCM6368_NAND_STATUS_MASK,
			priv->base + BCM6368_NAND_INT);

	return nfsnand_probe(dev, soc);
}

static const struct udevice_id bcm6368_nand_dt_ids[] = {
	{
		.compatible = "nfs,nand-bcm6368",
	},
	{ /* sentinel */ }
};
