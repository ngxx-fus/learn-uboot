...
struct nfstb_spi_plat {
	void *base[4];
};

struct nfstb_spi_priv {
	struct nfstb_hif_mspi_regs *regs;
	void *bspi;
	void *hif_spi_intr2;
	void *cs_reg;
	int default_cs;
...
static int nfstb_spi_of_to_plat(struct udevice *bus)
{
	struct nfstb_spi_plat *plat = dev_get_plat(bus);
	...
	char *names[BASE_LAST] = { "hif_mspi", "bspi", "hif_spi_intr2",
				   "cs_reg" };
	const phys_addr_t defaults[BASE_LAST] = { NFSTB_HIF_MSPI_BASE,
						  NFSTB_BSPI_BASE,
						  NFSTB_HIF_SPI_INTR2,
						  NFSTB_CS_REG };
	...
	for (i = 0; i < BASE_LAST; i++) {
		...
		ret = fdt_get_named_resource(fdt, node, "reg", "reg-names",
					     names[i], &resource);
		if (ret) {
			printf("%s: Assuming NFSTB SPI %s address 0x0x%p\n",
			       __func__, names[i], (void *)defaults[i]);
		} else {
			plat->base[i] = (void *)resource.start;
			debug("NFSTB SPI %s address: 0x0x%p\n",
...
static void nfstb_spi_hw_set_parms(struct nfstb_spi_priv *priv)
{
	writel(SPBR_MIN, &priv->regs->spcr0_lsb);
	writel(BITS_PER_WORD << 2 | SPI_MODE_3, &priv->regs->spcr0_msb);
}

static void nfstb_spi_enable_interrupt(void *base, u32 mask)
{
...
static void nfstb_spi_disable_interrupt(void *base, u32 mask)
{
...
static void nfstb_spi_clear_interrupt(void *base, u32 mask)
{
...
static int nfstb_spi_probe(struct udevice *bus)
{
	struct nfstb_spi_plat *plat = dev_get_plat(bus);
	struct nfstb_spi_priv *priv = dev_get_priv(bus);
	...
	/* Set up interrupts. */
	nfstb_spi_disable_interrupt(priv->hif_spi_intr2, 0xffffffff);
	nfstb_spi_clear_interrupt(priv->hif_spi_intr2, 0xffffffff);
	nfstb_spi_enable_interrupt(priv->hif_spi_intr2,
	...
	nfstb_spi_hw_set_parms(priv);

	return 0;
}

static void nfstb_spi_submit(struct nfstb_spi_priv *priv, bool done)
{
	debug("WR NEWQP: %d\n", 0);
	writel(0, &priv->regs->newqp);
...
static int nfstb_spi_wait(struct nfstb_spi_priv *priv)
{
	...
	nfstb_spi_clear_interrupt(priv->hif_spi_intr2,
				   HIF_SPI_INTR2_CPU_SET_MSPI_DONE_MASK);

	return 0;
}

static int nfstb_spi_xfer(struct udevice *dev, unsigned int bitlen,
			   const void *dout, void *din, unsigned long flags)
{
	...
	struct nfstb_spi_priv *priv = dev_get_priv(bus);
	struct nfstb_hif_mspi_regs *regs = priv->regs;
	...
	if (!(flags & (SPI_XFER_BEGIN | SPI_XFER_END))) {
		if (priv->saved_din_addr == din) {
			...
			ret = nfstb_spi_xfer(dev, priv->saved_cmd_len * 8,
					      priv->saved_cmd, NULL,
					      SPI_XFER_BEGIN);
			if (ret) {
	...
	while (rx_len > 0) {
		...
		nfstb_spi_submit(priv, tx_len == 0);

		if (nfstb_spi_wait(priv) == -ETIMEDOUT) {
			printf("%s: Timed out\n", __func__);
			return -ETIMEDOUT;
		}
...
static int nfstb_spi_set_speed(struct udevice *dev, uint speed)
{
	return 0;
}

static int nfstb_spi_set_mode(struct udevice *dev, uint mode)
{
	return 0;
}

static const struct dm_spi_ops nfstb_spi_ops = {
	.xfer		= nfstb_spi_xfer,
	.set_speed	= nfstb_spi_set_speed,
	.set_mode	= nfstb_spi_set_mode,
};

static const struct udevice_id nfstb_spi_id[] = {
	{ .compatible = "brcm,spi-brcmstb" },
	{ }
};

U_BOOT_DRIVER(nfstb_spi) = {
	.name				= "nfstb_spi",
	.id				= UCLASS_SPI,
	.of_match			= nfstb_spi_id,
	.ops				= &nfstb_spi_ops,
	.of_to_plat		= nfstb_spi_of_to_plat,
	.probe				= nfstb_spi_probe,
	.plat_auto	= sizeof(struct nfstb_spi_plat),
	.priv_auto		= sizeof(struct nfstb_spi_priv),
};
