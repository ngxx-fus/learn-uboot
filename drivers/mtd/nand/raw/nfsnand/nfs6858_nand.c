...
static bool bcm6858_nand_intc_ack(struct nfsnand_soc *soc)
{
...
static void bcm6858_nand_intc_set(struct nfsnand_soc *soc, bool en)
{
	struct bcm6858_nand_soc *priv =
			container_of(soc, struct bcm6858_nand_soc, soc);
...
static int bcm6858_nand_probe(struct udevice *dev)
{
	...
	nfsnand_writel(0, priv->base + BCM6858_NAND_INT_EN);
	nfsnand_writel(0, priv->base + BCM6858_NAND_INT);
