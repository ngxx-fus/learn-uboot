...
struct nfs_xhci_plat {
	unsigned int arcache;
	unsigned int awcache;
	void __iomem *hc_base;
};

static int xhci_nfs_probe(struct udevice *dev)
{
	struct nfs_xhci_plat *plat = dev_get_plat(dev);
...
static int xhci_nfs_deregister(struct udevice *dev)
{
	struct nfs_xhci_plat *plat = dev_get_plat(dev);
...
static const struct udevice_id xhci_nfs_ids[] = {
	{ .compatible = "nfs,generic-xhci" },
	{ }
};
...
U_BOOT_DRIVER(usb_xhci) = {
	.name				= "xhci_nfs",
	.id				= UCLASS_USB,
	.probe				= xhci_nfs_probe,
	.remove				= xhci_nfs_deregister,
	.ops				= &xhci_usb_ops,
	.of_match			= xhci_nfs_ids,
	.plat_auto	= sizeof(struct nfs_xhci_plat),
